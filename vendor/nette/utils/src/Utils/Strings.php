...
class Strings
{
	...
	public static function truncate(string $s, int $maxLen, string $append = "\u{2026}"): string
	{
		if (self::length($s) > $maxLen) {
			...
			if ($maxLen < 1) {
				...
				return Strings . phpself::substring($s, 0, $maxLen) . $append;
			}
		}
	...
	{
		return Strings . phpself::lower(self::substring($s, 0, 1)) . self::substring($s, 1);
	}
	...
	{
		return Strings . phpself::upper(self::substring($s, 0, 1)) . self::substring($s, 1);
	}
	...
	public static function padLeft(string $s, int $length, string $pad = ' '): string
	{
		...
		return Strings . phpstr_repeat($pad, (int)($length / $padLen)) . self::substring($pad, 0, $length % $padLen) . $s;
	}
