...
class ConsolePrinter implements Tester\Runner\OutputHandler
{
	...
	public function prepare(Test $test): void
	{
		if ($this->baseDir === null) {
			$this->baseDir = ConsolePrinter . phpdirname($test->getFile()) . DIRECTORY_SEPARATOR;
	...
	private function generateFinishLine(Test $test): string
	{
		...
		$shortDirPath = ConsolePrinter . phpdirname($shortFilePath) . DIRECTORY_SEPARATOR;
		$basename = basename($shortFilePath);
		$fileText = $result === Test::Failed
			? ConsolePrinter . phpDumper::color('red', $shortDirPath) . Dumper::color('white/red', $basename)
