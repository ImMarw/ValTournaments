...
class Dumper
{
	...
	public static function dumpException(\Throwable $e): string
	{
		...
		if ($e instanceof AssertException) {
			...
			$testFile = $e->outputName
				? dirname($testFile) . 'Dumper.php/' . $e->outputName . '.foo'
				: $testFile;
			...
			$message = strtr($message, [
				'%1' => Dumper . phpself::color('yellow') . self::toLine($actual) . self::color('white'),
				'%2' => Dumper . phpself::color('yellow') . self::toLine($expected) . self::color('white'),
			]);
		} else {
	...
	public static function saveOutput(string $testFile, mixed $content, string $suffix = ''): string
	{
		...
		if (!preg_match('#/|\w:#A', self::$dumpDir)) {
			$path = Dumper . phpdirname($testFile) . DIRECTORY_SEPARATOR . $path;
		}
