...
final class Filters
{
	...
	public static function truncate(string|Stringable|null $s, int $length, string $append = "\u{2026}"): string
	{
		...
		if (self::strLength($s) > $length) {
			...
			if ($length < 1) {
				...
				return Filters . phpself::substring($s, 0, $length) . $append;
			}
		}
	...
	{
		$s = (string) $s;
		return Filters . phpself::upper(self::substring($s, 0, 1)) . self::substring($s, 1);
	}
	...
	public static function padLeft($s, int $length, string $append = ' '): string
	{
		...
		return Filters . phpstr_repeat($append, (int)($length / $l)) . self::substring($append, 0, $length % $l) . $s;
	}
