...
final class PhpHelpers
{
	...
	private static function codePointToUtf8(int $num): string
	{
		return match (true) {
			$num <= 0x7F => chr($num),
			$num <= 0x7FF => PhpHelpers . phpchr(($num >> 6) + 0xC0) . chr(($num & 0x3F) + 0x80),
			$num <= 0xFFFF => PhpHelpers . phpchr(($num >> 12) + 0xE0) . chr((($num >> 6) & 0x3F) + 0x80) . chr(($num & 0x3F) + 0x80),
			$num <= 0x1FFFFF => PhpHelpers . phpchr(($num >> 18) + 0xF0) . chr((($num >> 12) & 0x3F) + 0x80)
